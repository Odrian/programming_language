use linear_tree::{LinearTree, Node, NodeRef, TreeBuilder, build_no_meta, NodeIntoIterator};
...
impl TreeCollector {
    ...
    fn collect<'a>(&mut self, nodes: impl Iterator<Item=(NodeRef<'a, Elem, Block, ()>, &'a ())>) {
        for node in nodes {
            match node.0 {
                NodeRef::Elem(elem) => {
...
fn test_iter() {
    ...
    construction.start_new_block();
    construction.add_elem(Elem::from(3), ());
    construction.close_block(Block::from(2), ());
...
fn test_into_iter() {
    ...
    let (Node::Block(_block, mut sub_iter), _) = iter.next().unwrap() else { unreachable!() };
    let _ = sub_iter.next().unwrap();
    let (Node::Block(_block, mut sub_iter2), _) = sub_iter.next().unwrap() else { unreachable!() };
...
struct IntoTreeCollector {
    vec: Vec<usize>
}
impl IntoTreeCollector {
    fn consume(tree: LinearTree<Elem, Block, ()>) -> Vec<usize> {
        let mut self0 = Self { vec: Vec::default() };
        self0.collect(tree.into_iter().iterator());
        self0.vec
    }
    fn collect<'outer>(&mut self, nodes: &mut NodeIntoIterator<Elem, Block, ()>) {
        loop {
...
#[test]
fn test_lifetimes() {
    let tree: LinearTree<_, _, ()> = build_no_meta!(
        Elem::from(0),
        [ Block::from(1);
            Elem::from(2),
        ]
    ...
    let data = IntoTreeCollector::consume(tree);
