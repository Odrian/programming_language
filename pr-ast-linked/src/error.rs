...
impl LinkingError {
    ...
    pub fn import_no_file(from_file: String, current_module: &String, range: Range) -> Diagnostic {
        DiagnosticString::new(
            format!("cant find file {from_file}, current module: {current_module}")
        ).to_diag(range)
    }
    pub fn import_no_name(name: String, file: &String, range: Range) -> Diagnostic {
        DiagnosticString::new(
            format!("cant find '{name}' in file {file}")
        ).to_diag(range)
    }
    pub fn import_global_name_overlap(range: Range, file: &String) -> Diagnostic {
        DiagnosticString::new(
            format!("name overloading in file {file}")
        ).to_diag(range)
    }
}
